...
def prediction(
    ...
    mu_theta  = mu[2]
    u_x       = u[0]                         # translational component
    u_theta   = u[2]                         # rotational component

    # Jacobian A: identity everywhere except the 3x3 robot block
    A = np.eye(dim_mu)
    A[0:3, 0:3] = np.array([
        [1, 0, -u_x * np.sin(mu_theta)],
        [0, 1,  u_x * np.cos(mu_theta)],
        [0, 0,  1                      ],
    ])
    ...
    sigma_u_const = 0.1
    sigma_u = np.array([
        [sigma_u_const**2 + u_x**2,     0                              ],
        [0,                              sigma_u_const**2 + u_theta**2  ],
    ])
